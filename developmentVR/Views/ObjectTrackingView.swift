        ...
        RealityView { content, attachments in
            ...
            Task {
                ...
                for await anchorUpdate in objectTracking.anchorUpdates {
                    ...
                    case .added:
                        guard let selectedCaseGroup = appState.selectedCaseGroup else {
                            print("No selected case group for visualization.")
                            return
                        }
                        
                        // Assuming the first USDZ entity in the selectedCaseGroup is the one to visualize
                        let model = selectedCaseGroup.usdzEntities.first(where: { $0 != nil }) as? ModelEntity
                        
                        let visualization = ObjectAnchorVisualization(for: anchor, withModel: model, caseGroup: selectedCaseGroup)
